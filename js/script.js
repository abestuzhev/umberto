...
$(document).ready(function () {
    ...
    $('.cart_step_one').click(function (e) { // LEEFT
        e.preventDefault();
        var _this = $(this);
        if (path === '/basket/') {
            console.log(path);
            var data = {'action': 'check_cart', 'sessid': $('#sessid').val()};
            $.post('/local/ajax/cart.php', data).done(function (data) {
                var res = JSON.parse(data);
                console.log(res);
                if (res.price * 1 < 400) {
        ...
        if ($('#checkout_form')[0].checkValidity()) {
            ...
            $.post('/local/ajax/calcCheckRes.php', data).done(function (datar) {
                ...
                if (res.problem === null) {
                    ...
                    var maxbonus = Math.floor(res.maxBonus);
                    var bonus = Math.floor(res.bonus);
                    var sum = res.fullSum;
                    var cartsum = res.fullSum;
                    ...
                    if (dis !== undefined && dis !== null) {
                        ...
                        $('#all_cart_summ').text(cartsum * 1 - dis);
                        $('#all_summ').text(sum * 1);

                    } else {
                        $('#discount_summ').fadeOut();
                        $('#all_cart_summ').text(cartsum);
                        $('#all_summ').text(sum * 1 - delsum);
